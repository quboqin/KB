...
// 连线配置：引入 anchor 锚点机制，控制线条只在边缘对接
const connections = [
  // 外部物理网络连接
  { from: 'internet', to: 'modem', type: 'fiber', anchor: 'right-left' },
  { from: 'modem', to: 'main-router', type: 'fiber', anchor: 'right-left' }, // 拨号线 (WAN)
  { from: 'modem', to: 'main-router', type: 'management', anchor: 'bottom-bottom', route: 'bridge-bottom' }, // 新增局域网直通线 (LAN)
...
// --- Format Text output from Gemini ---
const formatReportText = (text) => {
  const formatted = text
    ...
    .replace(/^### (.*$)/gim, '<h4 class="text-base font-bold text-slate-200 mt-3 mb-1">$1</h4>');
...
export default function App() {
  ...
  const getLineProps = (type) => {
    ...
    switch (type) {
      ...
      case 'management': // 新增物理 LAN 管理互通线
      ...
      default: 
        return { className: baseClass + "stroke-slate-600 stroke-[2px]" };
    }
  };
  ...
  const handleAnalyzeNetwork = async () => {
    ...
    const apiKey = ""; 
    const promptText = `
    你是一个资深的网络安全工程师。请分析以下智能家居与HomeLab混合网络拓扑数据，并用专业的中文给我一份分析报告。
    数据结构:
    ${JSON.stringify(devices, null, 2)}
    最新进展：已经在光猫和主路由之间拉了物理网线打通LAN管理，并且已经成功对 OpenClaw Ubuntu 配置了外网的 SSH 端口映射。
    请包含：1. 架构评估。2. 安全隐患预警（外网SSH暴露风险、旁路由、虚拟机网关等）。3. 优化建议。
    `;

    const payload = {
      contents: [{ parts: [{ text: promptText }] }],
      systemInstruction: { parts: [{ text: "你是一个专业的IT架构师和网络顾问。" }] }
    };
    ...
    try {
      ...
      else throw new Error("无法从 Gemini 解析响应内容");
    } catch (error) {
      setAiError("无法连接到 AI 大模型进行分析。请稍后重试或检查网络状态。");
    } finally {
  ...
  const handleExportPNG = async () => {
    ...
    try {
      ...
      link.download = 'HomeLab_Network_Topology.png';
      link.href = dataUrl;
      link.click();
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="max-w-[1700px] mx-auto min-w-[1400px] relative pb-16" ref={containerRef}>
        ...
        <div className="flex justify-between items-end mb-16 z-30 relative bg-slate-800/30 p-6 rounded-2xl border border-slate-700/50 backdrop-blur-md shadow-lg">
          <div>
            ...
            <p className="text-slate-400 text-sm font-medium">遵循 layout.png 宏观矩阵 & pve.png 微观结构的智能全景大屏</p>
          </div>
          
          <div className="flex flex-col items-end gap-4">
            <div className="flex items-center gap-3" data-export-ignore="true">
              <button 
                onClick={handleExportPNG}
                disabled={isExporting}
                className="group relative flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 px-4 py-2.5 rounded-xl border border-slate-600 shadow-md transition-all overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
                title="导出高清拓扑图为 PNG"
              >
                {isExporting ? (
                   <div className="w-4 h-4 border-2 border-slate-400 border-t-white rounded-full animate-spin"></div>
                ) : (
                  <CameraIcon size={18} className="text-slate-300 group-hover:text-white" />
                )}
                <span className="font-bold tracking-wide text-sm">{isExporting ? '高清渲染中...' : '导出 PNG'}</span>
              </button>

              <button 
                onClick={handleAnalyzeNetwork}
                className="group relative flex items-center gap-2 bg-gradient-to-r from-emerald-600/80 to-teal-600/80 hover:from-emerald-500 hover:to-teal-500 text-white px-5 py-2.5 rounded-xl border border-emerald-400/30 shadow-[0_0_20px_rgba(16,185,129,0.2)] transition-all overflow-hidden"
              >
                <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                <Sparkles size={18} className="text-emerald-200 group-hover:animate-pulse relative z-10" />
                <span className="font-bold tracking-wide relative z-10 text-sm">✨ AI 架构评估</span>
              </button>
            </div>
        ...
        <div className="grid grid-cols-[140px_380px_440px_380px] gap-x-10 gap-y-16 relative z-10 items-center">
          ...
          <div className="col-start-3 flex justify-center w-full relative">
            <ProNodeCard {...devices.core[2]} width="w-56" /> {/* 旁路由 */}
          </div>
          <div className="col-start-4 flex justify-center w-full self-end relative pb-4">
             ...
             <div id="group-bluetooth" className="border-2 border-slate-600/50 rounded-2xl p-5 bg-slate-800/30 w-full shadow-lg relative">
                <h3 className="text-slate-300 font-bold mb-4 flex items-center gap-2 text-sm">
                  <Zap size={16} className="text-blue-400"/> 蓝牙的设备
                </h3>
          ...
          <div className="col-start-2 flex justify-center w-full relative">
            <ProNodeCard {...devices.core[0]} width="w-56" /> {/* 光猫 */}
          </div>
          <div className="col-start-3 flex justify-center w-full relative">
            <div className="absolute -inset-10 bg-indigo-900/10 rounded-[3rem] blur-xl -z-10"></div>
            <ProNodeCard {...devices.core[1]} glow={true} width="w-72" /> {/* 主路由 */}
          </div>
          ...
          <div className="col-start-3 flex justify-center w-full self-start relative">
            <ProNodeCard {...devices.core[3]} width="w-56" /> {/* Mesh */}
          </div>
          ...
          <div className="col-start-3 col-span-2 flex justify-start w-full self-start relative mt-4">
            ...
            {/* PVE 展开视图容器 (严格模拟 pve.png 并且横跨两列) */}
      ...
      {showAiModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm" data-export-ignore="true">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-5 border-b border-slate-800 bg-slate-800/50">
              <div className="flex items-center gap-3">
                <div className="bg-emerald-500/20 p-2 rounded-lg"><Bot className="text-emerald-400" size={24} /></div>
                <div>
                  <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2">
                    Gemini 网络架构深度分析 <Sparkles size={16} className="text-amber-400" />
                  </h2>
                  <p className="text-xs text-slate-400">基于最新 PVE/NAS 微观展开模型进行诊断</p>
                </div>
              </div>
              <button onClick={() => setShowAiModal(false)} className="text-slate-400 hover:text-white hover:bg-slate-700 p-2 rounded-lg"><X size={20} /></button>
            </div>
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-gradient-to-b from-slate-900 to-[#0B1120]">
              {isAnalyzing ? (
                <div className="flex flex-col items-center justify-center h-64 gap-6">
                  <div className="relative flex items-center justify-center">
                    <div className="absolute inset-0 border-t-2 border-emerald-500 rounded-full animate-spin w-16 h-16 opacity-70"></div>
                    <Bot size={32} className="text-slate-500 animate-pulse" />
                  </div>
                  <div className="text-center">
                    <p className="text-emerald-400 font-medium tracking-widest text-sm animate-pulse">正在深度解析架构与虚拟化拓扑...</p>
                    <p className="text-slate-500 text-xs mt-2">评估最新暴露的外网SSH及LAN互通策略的安全性</p>
                  </div>
                </div>
              ) : aiError ? (
                <div className="flex flex-col items-center justify-center h-48 text-rose-400 gap-3">
                  <AlertTriangle size={40} /><p>{aiError}</p>
                </div>
              ) : (
