...
import {
  ...
  Activity, Zap, Network, AlertTriangle, Link2,
} from 'lucide-react';
...
const devices = {
  ...
  hub: { id: 'dev-24g-hub', name: '中枢网关 Hub', ip: '192.168.123.172', mac: '94:F8:27:7F:7C:C8', icon: Cpu, color: 'text-amber-400' } as DeviceBase,
  bluetooth: [
...
const ProNodeCard = ({ id, name, ip, mac, desc, gateway, warning, badge, icon: Icon, color, bg, glow, width = "w-64" }: DeviceBase & { width?: string }) => {
...
const CompactDeviceCard = ({ id, name, ip, mac, icon: Icon, color }: DeviceBase) => {
  const SafeIcon = Icon || Box;
  return (
