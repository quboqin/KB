...
#### Step 3.1 — 常见失败原因（2026.2.x 之后容易踩）

如果你发现脚本“跑了但没 patch 上”，通常是下面两个原因之一：

**原因 A：全局 OpenClaw 目录不是 git 仓库**

`$(npm root -g)/openclaw` 是 npm/pnpm 安装出来的文件夹，默认 **没有 `.git/`**。
此时 `git apply` 可能无法正常工作（尤其是 patch 里包含 `create mode` / 新文件）。

**原因 B：sudo 需要密码，但脚本运行环境没有交互 TTY**
...
#### Step 3.2 — 推荐修复：用“复制文件 + 编译 JS”替代 git apply

适用场景：你 patch 的是 **extensions/feishu** 这种 TypeScript 插件代码。

> 背景：Feishu 插件入口在 `extensions/feishu/package.json` 里声明 `extensions: ["./index.ts"]`，但运行时实际会 import `./src/*.js`。
> 所以只复制 `.ts` 往往不够，需要把对应文件编译成 `.js`。
>
> 如果你已经确认报错的根因是“缺少 `.js` 文件”，通常说明 `calendar.ts` 没有被正确编译/加载。这时不必整仓重建 OpenClaw，先在全局扩展目录里把缺失的文件补编译出来即可。

**Step 1 — 复制改动文件到全局 OpenClaw**

```bash
GLOBAL_OPENCLAW="$(npm root -g)/openclaw"

# 以 feishu calendar patch 为例
cp ~/magic/vibe-coding/openclaw/extensions/feishu/src/calendar.ts \
  "$GLOBAL_OPENCLAW/extensions/feishu/src/calendar.ts"

# index.ts 建议直接对照本地仓库内容同步（避免手工 sed 破坏 export 块）
cp ~/magic/vibe-coding/openclaw/extensions/feishu/index.ts \
  "$GLOBAL_OPENCLAW/extensions/feishu/index.ts"
```

**Step 2 — 快速修复：只补编译缺失的 `calendar.js`**

如果你的目标只是先让日历功能恢复，而 `index.js` 已经存在，可以直接执行：

```bash
cd /home/openclaw/.nvm/versions/node/v22.22.0/lib/node_modules/openclaw/extensions/feishu
npx tsc src/calendar.ts \
  --outDir . \
  --module esnext \
...
# 确认文件已经生成
ls -la src/calendar.js
```

这一步对应的是“选项 2：手动把 `calendar.ts` 编译成 `calendar.js`”，优点是快，适合线上先恢复。

**Step 2.1 — 更稳妥的做法：连同入口一起重新编译**

```bash
cd "$GLOBAL_OPENCLAW/extensions/feishu"

# 安装编译器（只装在这个扩展目录，不污染全局）
...
npx tsc \
  ...
  --skipLibCheck \
  --outDir . \
  src/calendar.ts index.ts

# 关键检查：必须有这两个文件
ls -la index.js src/calendar.js
```

> 备注：编译过程中可能会看到某些 TS 警告（例如 `toSorted`），但只要 `src/calendar.js` 已经产出，就可以先继续验证；如果 `index.js` 也一并重编译成功会更稳妥。

**Step 3 — 重启网关并验证插件加载**

```bash
openclaw gateway restart
openclaw status | head -n 20
openclaw skills list | rg -i "feishu_calendar|feishu_"
```
